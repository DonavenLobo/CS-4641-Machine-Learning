...
class dlnet:
    ...
    def _dropout(u, prob):
        ...
        dropout_mask = np.random.choice([0, 1], size=u.shape, p=[prob, 1 - prob]) # drop out
        drop = u * dropout_mask
        scale = 1 / (1 - prob)
        u_after_dropout = drop * scale
    ...
    def compute_gradients(self, y, yh, use_dropout):
       ...
       # TODO: IMPLEMENT THIS METHOD
        ...
        dLoss_o2 = np.subtract(yh, y) / y.shape[1] # IMPLEMENT THIS LINE
        dLoss_u2 = np.multiply(dLoss_o2, self.dTanh(self.ch["u2"])) # IMPLEMENT THIS LINE
        dLoss_theta2 = np.matmul(dLoss_u2, self.ch["o1"].T) # IMPLEMENT THIS LINE
        dLoss_b2 = np.matmul(dLoss_u2, np.ones((dLoss_u2.shape[1], 1)))# IMPLEMENT THIS LINE
        dLoss_o1 = np.multiply(dLoss_u2, self.param["theta2"].T) # IMPLEMENT THIS LINE
        
        if use_dropout:
            dLoss_u1 = np.multiply(dLoss_o1, self.dL_Relu(self.alpha, self.ch["u1"])) * self.ch['mask'] * (1 / (1 - self.dropout_prob)) # IMPLEMENT THIS LINE
        else:
            dLoss_u1 = np.multiply(dLoss_o1, self.dL_Relu(self.alpha, self.ch["u1"])) # IMPLEMENT THIS LINE


        dLoss_theta1 = np.matmul(dLoss_u1, self.ch["X"].T) # IMPLEMENT THIS LINE
        dLoss_b1 = np.matmul(dLoss_u1, np.ones((dLoss_u1.shape[1], 1))) # IMPLEMENT THIS LINE
    ...
    def update_weights(self, dLoss, use_momentum):
        ...
        for layer in dLoss:
            if use_momentum:
                ...
                self.param[layer] = self.param[layer] - (self.lr * dLoss[layer]) # IMPLEMENT THIS LINE
    ...
    def gradient_descent(self, x, y, iter = 60000, use_momentum=False, local_test=False):
        ...
        **For LOCAL TEST append and print out loss at every iteration instead of every 1000th multiple.
        ...
        for i in range(iter):
            ...
            self.forward(x, self.use_dropout)

            if i % 1000 == 0:
                self.loss.append(self.nloss(y, self.ch["o2"]))

            self.backward(y, self.ch["o2"], self.use_dropout, self.use_momentum)
