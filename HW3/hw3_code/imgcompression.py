...
class ImgCompression(object):
    ...
    def svd(self, X: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:  # [4pts]
        ...
        """
        Dim = np.shape(X) # Find Dimension of X
        if len(Dim) == 2: # If black and white image
            U, S, VT = np.linalg.svd(X, full_matrices = True)
        else: # If color image
            #Extract dimentions
            N = np.shape(X)[1] 
            D = np.shape(X)[2]
            # Create empty matricies for return variables
            U = np.empty((3, N,N))
            S = np.empty((3, min(N, D)))
            VT = np.empty((3, D, D))
            for i in range(3): # For each color dimension
    ...
    def compress(
        ...
        """
        Dim = np.shape(U); # Find dimension of U
        if len(Dim) == 2: # B&W
            U_compressed = U[:, :k]
            S_compressed = S[:k]
            V_compressed = V[:k, :]
    ...
    def rebuild_svd(
        ...
        """
        Dim = np.shape(U_compressed); # Find dimension of one of the input arguments
        if len(Dim) == 2:
            Xrebuild = np.matmul(U_compressed, np.matmul(np.diag(S_compressed), V_compressed))
        else:
            N = np.shape(U_compressed)[1]
            D = np.shape(V_compressed)[2]
            Xrebuild = np.empty((3, N, D))
            for i in range(3):
    ...
    def compression_ratio(self, X: np.ndarray, k: int) -> float:  # [4pts]
        ...
        Dim = np.shape(X)
        if len(Dim) == 2:
            comp= k * (1 + np.shape(X)[0] + np.shape(X)[1])
            org = np.shape(X)[0] * np.shape(X)[1]
            compression_ratio = comp / org
        else:
            comp= k * (1 + np.shape(X)[1] +np.shape(X)[2])
            org = np.shape(X)[1] * np.shape(X)[2]
            compression_ratio = comp / org
    ...
    def recovered_variance_proportion(self, S: np.ndarray, k: int) -> float:  # [4pts]
        ...
        Dim = np.shape(S)
        if len(Dim) == 1:# B&W
            x = np.sum(np.power(S[:k], 2), axis = 0)
            y = np.sum(np.power(S, 2), axis = 0)
        else: # Color
            x = np.sum(np.power(S[:, :k], 2), axis = 1)
            y = np.sum(np.power(S, 2), axis = 1)
        recovered_var = x/y
