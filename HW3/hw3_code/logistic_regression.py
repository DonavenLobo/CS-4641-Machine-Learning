...
class LogisticRegression(object):
    ...
    def bias_augment(self, x: np.ndarray) -> np.ndarray: # [3pts]
        ...
        x_aug = np.concatenate((ones_col, x), axis = 1)
        return x_aug
    ...
    def predict_probs(self, x_aug: np.ndarray, theta: np.ndarray) -> np.ndarray: # [5pts]
        ...
        """
        input = np.dot(x_aug, theta)
        h_x = self.sigmoid(input)
        return h_x
    ...
    def predict_labels(self, h_x: np.ndarray) -> np.ndarray: # [2pts]
        ...
        """
        N = np.shape(h_x)[0] # Number of data points
        y_hat = np.empty(np.shape(h_x))
    ...
    def loss(self, y: np.ndarray, h_x: np.ndarray) -> float: # [3pts]
        ...
        """
        N = np.shape(y)[0]
        sum = np.sum((y * np.log(h_x)) + ((1 - y) * (np.log(1 - h_x))))
        loss = (-1 / N) * sum 
        return loss
    ...
    def gradient(self, x_aug: np.ndarray, y: np.ndarray, h_x: np.ndarray) -> np.ndarray: # [3pts]
        ...
        N= np.shape(y)[0]
        dist = h_x - y
        grad = (1 / N) * np.matmul(x_aug.T, dist)
        return grad
    ...
    def accuracy(self, y: np.ndarray, y_hat: np.ndarray) -> float: # [2pts]
        ...
        for err in dist_error:
            if err[0] != 0:
                count += 1
        ratio = count / np.size(dist_error)
        accuracy = 1 - ratio
        return accuracy
