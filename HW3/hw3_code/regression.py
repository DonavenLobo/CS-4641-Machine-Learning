...
class Regression(object):
    ...
    def rmse(self, pred: np.ndarray, label: np.ndarray) -> float:  # [5pts]
        ...
        Return:
            A float value
        """
        mean = np.mean(np.square(pred - label))
        rmse = np.sqrt(mean) 
        return rmse
    ...
    def construct_polynomial_feats(
        ...
        if np.ndim(x) == 1:
            D = 1
            feat = np.empty((N, degree + 1))
        else:
            D = np.shape(x)[1]
            feat = np.empty((N, degree + 1, D))           
    ...
    def linear_fit_closed(
        ...
        """
        inv = np.linalg.pinv(xtrain) # get pseudoinverse of x
        weight = np.dot(inv, ytrain)
        return weight
    ...
    def ridge_fit_closed(
        ...
        """
        term1 = np.dot(xtrain.T, xtrain)
        I = np.identity(xtrain.shape[1])
        I[0][0] = 0
        term2 = c_lambda * I
        inv = np.linalg.pinv(term1 + term2)
        pseudoTrans = np.dot(inv, xtrain.T)
        weight = np.dot(pseudoTrans, ytrain)
        return weight
    ...
    def ridge_cross_validation(
        ...
        """
        loss_per_fold = []
        inst_per_fold = int(np.shape(X)[0] / kfold) # number of instances per fold

        for i in range(kfold):
            x_new = np.concatenate((X[:i * inst_per_fold], X[(i + 1) * inst_per_fold:])) 
            y_new = np.concatenate((y[:i * inst_per_fold], y[(i + 1) * inst_per_fold:]))

            fit = self.ridge_fit_closed(x_new, y_new, c_lambda)
            instances = X[i * inst_per_fold:(i + 1) * inst_per_fold] 
            
            prediction = self.predict(instances, fit)
            labels = y[i * inst_per_fold:(i + 1) * inst_per_fold] # get true laber values to find rmse.
            rmse = self.rmse(prediction, labels)
